...
const Opcion = require('../models/opciones_respuesta');

const getOpciones = (req, res) => {
    return new Promise(function(resolve, reject) {
        let query = 'SELECT * FROM opciones_respuesta';
        let conditions = [];
        let values = [];
        ...
        if(req.query.ID_Opcion){
            conditions.push("opciones_respuesta.ID_Opcion = ?");
            values.push(req.query.ID_Opcion);
        }
        if(req.query.ID_Pregunta){
            conditions.push("opciones_respuesta.ID_Pregunta = ?");
...
const createOpciones = (req, res) => {
    return new Promise(function(resolve, reject) {
        ...
        connection.query('SELECT * FROM opciones_respuesta WHERE TextoOpcion = ? AND ID_Pregunta = ?', [TextoOpcion, ID_Pregunta], (error, results) => {
            if (error) {
                console.log(error);
            ...
            connection.query('INSERT INTO opciones_respuesta SET ?', nuevaOpcion, (error, results) => {
                if (error) {
                    console.log(error);
...
const updateOpcion = (req, res) => {
    return new Promise(function(resolve, reject) {
        ...
        connection.query('UPDATE opciones_respuesta SET ? WHERE ID_Opcion = ?', [req.body, id], (error, results) => {
            if (error) {
...
const deleteOpcion = (req, res) => {
    return new Promise(function(resolve, reject) {
        ...
        connection.query('SELECT * FROM opciones_respuesta WHERE ID_Opcion = ?', [id], (error, rows) => {
            if (error) {
                ...
                if (rows.length === 0) {
                    ...
                    connection.query('DELETE FROM opciones_respuesta WHERE ID_Opcion = ?', [id], (error, results) => {
                        if (error) {
                            console.log(error);
                            reject({ statusCode: 500, message: "Error al eliminar la opción de respuesta"});
                        } else {
